...
void Engine::FinishUpdate()
{
   ...
   // Activate/Desactivate cap 30 FPS
    if (GetInstance().input->GetKey(SDL_SCANCODE_F11) == KEY_DOWN && !capFps) {
        capFps = true;
        maxFrameDuration = 32;
    }
    else if (GetInstance().input->GetKey(SDL_SCANCODE_F11) == KEY_DOWN && capFps) {
        capFps = false;
        maxFrameDuration = 16;
    }
